...
function generate(nbRoom) {
  ...
  while (index < dungeon.nbRoom ) {
    ...
    if (currentRoom[direction.name]) { // If there is already a door for this direction.
      ...
      currentRoom = dungeon.map[targetRoom];
    } else if (!roomExist) {
      ...
      dungeon.map.push(nextRoom); // Add the new room to the map.
...
function lockNextDoor(dungeon, roomId) {
  var room = dungeon.getRoomById(roomId);
  var doors = room.getDoorsList();

  _.forEach(doors, function(door) {
    if(door.targetRoomId > roomId) {
      door.locked = true;
    }
  });

  var roomWithKeyId = Math.random() * roomId - 1;
  var roomWithKey = dungeon.getRoomById(roomWithKeyId);
  roomWithKey.objectList.push(new Key(roomId));
}

module.exports = {
  generate: generate,
  lockNextDoor: lockNextDoor,
};
