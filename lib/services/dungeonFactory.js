...
function generateLvl(nbRoom) {
  var dungeonLvl = new DungeonLvl({ nbRoom: nbRoom });
...
function lockRoom(dungeonLvl, roomId) {

  var roomToLock = dungeonLvl.getRoomById(roomId);
  ...
  var nextRoom = dungeonLvl.getRoomById(nextRoomId);
  ...
  var roomWithKey = dungeonLvl.getRoomById(roomWithKeyId);
  roomWithKey.objectList.push(new Key(roomId));
  return roomWithKeyId;
}

function setExit(dungeonLvl) {
  var randRoomExit = commons.randElement(dungeonLvl.map);
