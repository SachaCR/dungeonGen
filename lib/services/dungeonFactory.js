...
function generateDungeon(lvlNbRoomList) {
  if (!lvlNbRoomList) {
    throw new Error('Dungeon need an array of room number by level');
  }

  if (Object.prototype.toString.call( lvlNbRoomList ) !== '[object Array]') {
    throw new Error('lvlNbRoomList must be an array of integer');
  }

  var lvlList = [];

  lvlNbRoomList.forEach(function(lvlNbRoom, index) {
